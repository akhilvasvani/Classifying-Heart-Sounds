"""This script runs a BiGRU model on Dataset B of Peter Bentley's
...
from keras.layers import Activation, Dense, GRU, Bidirectional
...
def run_model(x_train_data, y_train_data, x_val_data, y_val_data, x_test_data,
    ...
    model.summary()
    model.save("saved_trained_models/BiGRU_model.hdf5")
...
def main():
    ...
    try:
        filepath = os.path.join('results', 'BiGRU_predictions')
    except OSError:
        if pathlib.Path(os.path.join('results', 'BiGRU_predictions')).resolve(strict=True):
            pass
        else:
            raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT),
                                    os.path.join('results', 'BiGRU_predictions'))
