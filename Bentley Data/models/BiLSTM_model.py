"""This script runs a BiLSTM model on Dataset B of Peter Bentley's
...
from keras.layers import Activation, Dense, LSTM, Bidirectional
...
def train_validate_test_split(arr, train_percent=0.75, validate_percent=0.15):
    ...
    Raises:
        ValueError: if all the percentages (training, validation, and testing)
                    do not add up to 1.
    """
...
def preprocess_data():
    ...
    murmur_labels_train, murmur_labels_val, \
    ...
    extrasystole_train, \
    extrasystole_val, \
    extrasystole_test = train_validate_test_split(extrasystole_sounds)
    ...
    return x_train, y_train, x_val, y_val, x_test, y_test, \
           np.array(x_predict), x_predict_filenames


def run_model(x_train_data, y_train_data, x_val_data, y_val_data, x_test_data,
              y_test_data, x_predict_data):
    """
    BiLSTM model that takes in the training data, validation data, and testing
    ...
    model.summary()
    model.save("saved_trained_models/BiLSTM_model.hdf5")
...
def main():
    ...
    Attains results from running the BiLSTM model, and then creates dictionary
    ...
    x_predict, x_predict_filenames = preprocess_data()
    ...
    try:
        filepath = os.path.join('results', 'BiLSTM_predictions')
    except OSError:
        if pathlib.Path(os.path.join('results', 'BiLSTM_predictions')).resolve(strict=True):
            pass
        else:
            raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT),
                                    os.path.join('results', 'BiLSTM_predictions'))
